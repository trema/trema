/*
 * Sample redirectable routing switch application.
 *
...
handle_packet_in( uint64_t datapath_id, uint32_t transaction_id,
  ...
  if ( in_port <= OFPP_MAX ) {
    if ( port == NULL && !lookup_fdb( routing_switch->fdb, src, &datapath_id, &in_port ) ) {
      debug( "ignore this packet_in (not external port)" );
      return;
    }
  }
...
void
usage() {
  topology_service_interface_usage( get_executable_name(), "Redirectable Routing Switch.", option_description );
}
